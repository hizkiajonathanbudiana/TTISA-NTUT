    ...
    useEffect(() => {
        ...
        const performCheckIn = async () => {
            try {
                ...
                // Cek jika error.context adalah Response object
                if (error.context && typeof error.context.json === 'function') {
                    try {
                        // Kita harus 'await' untuk membaca body dari Response
                        const errorBody = await error.context.json();

                        if (errorBody && errorBody.error) {
                            // Ini adalah pesan error dari edge function kamu!
                            errorMessage = errorBody.error;
                        } else {
                            // Fallback jika body JSON tidak ada 'error'
                            errorMessage = error.message;
                        }
                    } catch (jsonError) {
                        // Gagal parse JSON, fallback ke error.message
                        errorMessage = error.message;
                    }
                } else if (error.message) {
                    // Fallback jika error.context bukan Response
                    errorMessage = error.message;
                }
