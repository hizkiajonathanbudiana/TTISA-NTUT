...
// //     const { data: events } = useQuery<Event[]>({ queryKey: ['upcoming_events'], queryFn: async () => { const { data, error } = await supabase.from('events').select('id, slug, title_en, title_zh_hant, start_at, location').gte('start_at', new Date().toISOString()).order('start_at', { ascending: true }).limit(3); if (error) throw new Error(error.message); return data; } });
...
// //             {testimonials && testimonials.length > 0 && (<AnimatedSection className="py-20"><div className="container mx-auto px-6 text-center"><h2 className="text-3xl font-bold mb-12 text-text-primary">From Our Members</h2><div className="grid grid-cols-1 md:grid-cols-3 gap-8">{testimonials.map(t => (<motion.div key={t.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}><div className="bg-white/50 p-6 rounded-lg shadow-lg h-full"><p className="text-text-secondary italic">"{t.comment}"</p><div className="flex items-center justify-center mt-4"><img src={t.avatar_url ? t.avatar_url : `https://api.dicebear.com/8.x/initials/svg?seed=${t.english_name}`} alt={t.english_name || ''} className="w-10 h-10 rounded-full mr-3" /><p className="font-bold text-text-primary">{t.english_name}</p></div></div></motion.div>))}</div></div></AnimatedSection>)}
// //         </div>
// //     );
// // };
