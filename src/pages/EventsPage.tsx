...
const fetchEvents = async ({ pageParam = 0, filter }: { pageParam: number, filter: 'upcoming' | 'past' }) => {
    const { data, error } = await supabase.functions.invoke('get-paginated-events', {
        body: { pageParam, filter }
    });
    if (error) {
        let specificMessage = "Error fetching events.";
            ...
            <section className="py-16 md:py-24 container mx-auto px-4">
                ...
                {upcomingEvents.length > 0 ? (
                    <InfiniteScroll
                        ...
                        hasMore={!!hasNextUpcoming}
                        loader={<h4 className="text-center text-text-secondary py-8">Loading more...</h4>}
                        endMessage={<p className="text-center text-text-secondary py-8">No more upcoming events.</p>}
            ...
            <section className="py-16 md:py-24 container mx-auto px-4">
                ...
                {pastEvents.length > 0 ? (
                    <InfiniteScroll
                        ...
                        hasMore={!!hasNextPast}
                        loader={<h4 className="text-center text-text-secondary py-8">Loading more...</h4>}
                        endMessage={<p className="text-center text-text-secondary py-8">No more past events.</p>}
                        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"
                    >
                        {pastEvents.map(event => <EventCard key={event.id} event={event} />)}
                    </InfiniteScroll>
