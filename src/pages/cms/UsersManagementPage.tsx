...
// type UserProfile = { roles: { name: 'admin' | 'organizer' | 'member'; } | null; };
...
//     const { data: roles, isLoading: isLoadingRoles } = useQuery<Role[]>({ queryKey: ['roles'], queryFn: async () => { const { data, error } = await supabase.from('roles').select('*'); if (error) throw new Error(error.message); return data; } });

//     const { data: currentUserProfile } = useQuery<UserProfile | null>({
//         queryKey: ['current_user_profile', currentUser?.id],
//         queryFn: async () => {
//             if (!currentUser) return null;
//             const { data, error } = await supabase.from('users').select('roles(name)').eq('id', currentUser.id).single();
...
//     const updateRoleMutation = useMutation({ mutationFn: async ({ userId, roleId }: { userId: string, roleId: string }) => { const { error } = await supabase.from('users').update({ role_id: roleId }).eq('id', userId); if (error) throw error; }, onSuccess: () => { toast.success("User role updated successfully!"); setEditingUser(null); queryClient.invalidateQueries({ queryKey: ['cms_users'] }); }, onError: (error) => toast.error(error.message), });
...
//     const isAdmin = currentUserProfile?.roles?.name === 'admin';
...
//                                 <tr key={user.user_id}>
//                                     <td data-label="Name">
//                                         <Link to={`/cms/users/${user.user_id}`} className="flex items-center gap-3 hover:underline text-primary">
//                                             <img src={user.avatar_url ? `${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/avatars/${user.avatar_url}` : `https://api.dicebear.com/8.x/initials/svg?seed=${user.english_name || user.email}`} alt="avatar" className="w-8 h-8 rounded-full" />
//                                             <span>{user.english_name || 'N/A'}</span>
//                                         </Link>
//                                     </td>
...
//                                             <CmsActionButton variant="secondary" onClick={() => setEditingUser(user)}>
//                                                 Edit Role
//                                             </CmsActionButton>
//                                         )}
//                                     </td>
...
type Role = { id: string; name: string; };
type UserDetails = { user_id: string; email: string | null; english_name: string | null; student_id: string | null; avatar_url: string | null; role_id: string | null; role_name: string | null; };
type UserProfile = { roles: { name: 'admin' | 'organizer' | 'member' | 'developer'; } | null; };
