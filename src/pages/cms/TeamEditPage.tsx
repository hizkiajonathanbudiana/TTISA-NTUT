...
// // // type UserProfile = { user_id: string; english_name: string | null; users: { email: string } };
...
// // //         queryFn: async () => {
// // //             const { data, error } = await supabase.from('teams').select('*, team_members(user_id, position, profiles(english_name))').eq('id', teamId!).single();
...
// // //     const { register, handleSubmit, reset, formState: { errors } } = useForm<TeamDetailsFormInputs>({ resolver: zodResolver(TeamDetailsSchema) });
// // //     useEffect(() => { if (team) reset(team); }, [team, reset]);

// // //     const updateDetailsMutation = useMutation({
// // //         mutationFn: async (formData: TeamDetailsFormInputs) => {
// // //             const { error } = await supabase.from('teams').update(formData).eq('id', teamId!);
...
// // //             if (error) throw error;
// // //         },
// // //         onSuccess: () => { toast.success('Member added!'); queryClient.invalidateQueries({ queryKey: ['cms_team', teamId] }); setSearch(''); setSearchResults([]); setPosition('Member'); },
...
// // //             {/* Team Details Form */}
// // //             <form onSubmit={handleSubmit(data => updateDetailsMutation.mutate(data))} className="lg:col-span-2 bg-white p-6 rounded-lg shadow-md space-y-4 mt-6">
// // //                 <h2 className="text-xl font-bold border-b pb-2 mb-4">Team Details</h2>
// // //                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
// // //                     <div><label className="block text-sm font-medium">Name (English)</label><input {...register('name_en')} className="mt-1 w-full p-2 border rounded-md" />{errors.name_en && <p className="text-sm text-system-danger">{errors.name_en.message}</p>}</div>
// // //                     <div><label className="block text-sm font-medium">Name (Chinese)</label><input {...register('name_zh_hant')} className="mt-1 w-full p-2 border rounded-md" />{errors.name_zh_hant && <p className="text-sm text-system-danger">{errors.name_zh_hant.message}</p>}</div>
// // //                 </div>
...
// // //                 <div className="flex items-center gap-2"><input type="checkbox" {...register('is_active')} className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" /><label>Team is Active</label></div>
...
// // //                         <div>
// // //                             <label className="block text-sm font-medium">Search by Name or Email</label>
// // //                             <input type="text" value={search} onChange={(e) => handleSearch(e.target.value)} className="mt-1 w-full p-2 border rounded-md" placeholder="Enter at least 2 characters" />
// // //                         </div>
// // //                         {searchResults.length > 0 &&
// // //                             <div className="border rounded-md bg-neutral-100 max-h-40 overflow-y-auto">
// // //                                 {searchResults.map(user => (
// // //                                     <div key={user.user_id} className="p-2 border-b last:border-b-0 cursor-pointer hover:bg-neutral-200" onClick={() => addMutation.mutate(user)}>
// // //                                         {user.english_name} ({user.users.email})
// // //                                     </div>
// // //                                 ))}
// // //                             </div>
// // //                         }
...
// // //                             <li key={member.user_id} className="flex justify-between items-center p-2 rounded-md hover:bg-neutral-100">
// // //                                 <span><strong>{member.profiles?.english_name}</strong> ({member.position})</span>
// // //                                 <button onClick={() => removeMutation.mutate(member.user_id)} className="text-xs font-semibold text-red-500 hover:underline">Remove</button>
// // //                             </li>
// // //                         ))}
// // //                     </ul>
// // //                 </div>
...
// // const TeamDetailsSchema = z.object({ name_en: z.string().min(1, 'English name is required'), name_zh_hant: z.string().min(1, 'Chinese name is required'), description_en: z.string().optional(), description_zh_hant: z.string().optional(), is_active: z.boolean(), });
// // type TeamDetailsFormInputs = z.infer<typeof TeamDetailsSchema>;
// // type Member = { user_id: string, position: string, profiles: { english_name: string | null } };
// // type Team = { id: string, team_members: Member[] } & TeamDetailsFormInputs;
...
// //     const { data: team, isLoading } = useQuery<Team>({ queryKey: ['cms_team', teamId], queryFn: async () => { const { data, error } = await supabase.from('teams').select('*, team_members(user_id, position, profiles(english_name))').eq('id', teamId!).single(); if (error) throw new Error(error.message); return data; }, enabled: !!teamId, });
// //     const { register, handleSubmit, reset } = useForm<TeamDetailsFormInputs>({ resolver: zodResolver(TeamDetailsSchema) });
// //     useEffect(() => { if (team) reset(team); }, [team, reset]);
...
// //     };
// //     const addMutation = useMutation({ mutationFn: async (user: UserProfile) => { const { error } = await supabase.from('team_members').insert({ team_id: teamId!, user_id: user.user_id, position }); if (error) throw error; }, onSuccess: () => { toast.success('Member added!'); queryClient.invalidateQueries({ queryKey: ['cms_team', teamId] }); setSearch(''); setSearchResults([]); setPosition('Member'); }, onError: (e: any) => toast.error(`Failed to add member: ${e.code === '23505' ? 'User is already in this team.' : e.message}`), });
...
// //                 <div className="bg-white p-6 rounded-lg shadow-md">
// //                     <h2 className="text-xl font-bold mb-4">Current Members ({team?.team_members.length || 0})</h2>
// //                     <ul className="space-y-2 max-h-60 overflow-y-auto">{team?.team_members.map(member => (<li key={member.user_id} className="flex justify-between items-center p-2 rounded-md hover:bg-neutral-100"><span><strong>{member.profiles?.english_name}</strong> ({member.position})</span><button onClick={() => removeMutation.mutate(member.user_id)} className="text-xs font-semibold text-red-500 hover:underline">Remove</button></li>))}</ul>
// //                 </div>
// //             </div>
// //         </div>
// //     );
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(data => updateDetailsMutation.mutate(data))} className="lg:col-span-2 bg-white p-6 rounded-lg shadow-md space-y-4 mt-6">
        ...
        <div><label className="block text-sm font-medium">Description (Chinese)</label><textarea {...register('description_zh_hant')} rows={3} className="mt-1 w-full p-2 border rounded-md" /></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium">Display Order</label><input type="number" {...register('display_order')} className="mt-1 w-full p-2 border rounded-md" /></div>
