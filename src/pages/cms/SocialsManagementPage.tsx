...
export const SocialsManagementPage = () => {
    ...
    // Query menggunakan Edge Function
    const { data: linksData, isLoading } = useQuery({
        queryKey: ['social_links', currentPage, searchTerm],
        queryFn: async () => {
            const { data, error } = await supabase.functions.invoke('get-cms-social-links', {
                body: {
                    page: currentPage,
                    limit: 20,
                    search: searchTerm
            ...
            if (error) throw new Error(error.message);
            return data; // data sudah berisi { data: [...], pagination: {...} }
        },
    ...
    // Mutasi menggunakan Edge Function
    const mutation = useMutation({
        mutationFn: async (formData: SocialFormInputs) => {
            if (!session) throw new Error("Not authenticated");
            const action = editingLink ? 'update' : 'create';
            const id = editingLink ? editingLink.id : undefined;

            const { error } = await supabase.functions.invoke('crud-social-link', {
                body: { action, id, data: formData }
            });
            if (error) throw error;
        },
        onSuccess: () => {
