...
//   const { data, error } = await supabase.functions.invoke('get-public-teams');
//   if (error) {
//     let specificMessage = "Error fetching teams.";
...
//   if (error) return <div className="text-center py-40 text-system-danger">Error: {(error as Error).message}</div>;

//   return (
//     <div className="bg-background">
//       <SEO
//         title="Our Team - TTISA NTUT"
//         description="Meet the dedicated team members of TTISA at National Taiwan University of Science and Technology. Learn about our leadership and the passionate individuals who make our community to life."
...
//                           <img src={member.profiles?.avatar_url ? member.profiles.avatar_url : `https://api.dicebear.com/8.x/initials/svg?seed=${member.profiles?.english_name}`} alt={member.profiles?.english_name || 'Member'} className="w-24 h-24 rounded-full mx-auto object-cover border-4 border-white/50 shadow-lg" />
...
export const TeamsPage = () => {
  ...
  if (error)
    return (
      <div className="text-center py-40 text-system-danger">
    ...
    <div className="bg-background">
      ...
      <section className="relative pt-40 pb-20 flex items-center justify-center text-center p-4 overflow-hidden">
        <div className="absolute inset-0 z-0 opacity-50">
          <div className="absolute top-[5%] left-[10%] w-72 h-72 lg:w-96 lg:h-96 bg-accent-purple rounded-full filter blur-3xl animate-pulse"></div>
        ...
        <div className="relative z-10 max-w-3xl">
          <motion.h1
            ...
            className="text-4xl md:text-6xl font-extrabold text-text-primary"
          >
            {t('teams.pageTitle')}
          </motion.h1>
          <motion.p
      ...
      <div className="container mx-auto px-4 py-16 space-y-16">
        {isLoading ? (
          ...
          teams.map((team: Team) => {
            const teamName =
              language === 'zh-HANT' && team.name_zh_hant
                ? team.name_zh_hant
                : team.name_en;
            const teamDescription =
              language === 'zh-HANT' && team.description_zh_hant
                ? team.description_zh_hant
                : team.description_en;
            return (
              <AnimatedSection key={team.id}>
                ...
                {/* pakai flex biar auto center berapapun jumlah member */}
                <div className="flex flex-wrap justify-center gap-8 mt-12">
                  {team.team_members.map((member: Member, i: number) => {
                    const position =
                      language === 'zh-HANT' && member.position_zh_hant
                        ? member.position_zh_hant
                        : member.position_en;
                    return (
                      <motion.div
                        key={`${team.id}-${member.profiles?.english_name}`}
                      ...
                      >
                        <img
                          src={
                            member.profiles?.avatar_url
                              ? member.profiles.avatar_url
                              : `https://api.dicebear.com/8.x/initials/svg?seed=${member.profiles?.english_name}`
                          }
                        ...
                        />
                        <h3 className="font-bold mt-3 text-text-primary">
                          {member.profiles?.english_name}
                        </h3>
